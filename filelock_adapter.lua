...
function wait_for_lock()
  ...
  while true do
    ...
    if success then
      ...
      -- Verify we actually have the lock by checking the lock file
      local lockInfo = lock:_readLockFile()
      if lockInfo and lockInfo.lockId == lock.lockId then
        return lock -- Return the lock object instead of file pointer
      else
        printf("%s\n", colortxt(sprintf("PID-%s: Lock verification failed, retrying", TXPID), "red"))
      end
    end
    
    -- Failed to acquire lock or verification failed, wait with exponential backoff
    printf2("%s\n", colortxt(sprintf("PID-%s: Waiting for lock for %s secs", TXPID, delay), "red"))
    sleep(delay)
    delay = math.min(delay * 2, 10)  -- Exponential backoff, capped at 10 seconds
  end
